package br.com.martinsdev.forumhub.infra.email;
...
public class EmailService {
    ...
    public void verificaoEmail(Usuario usuario) {
        String asssunto = "Aqui está seu link para verificar o email!";
        String conteudo = gerarConteudoEmail("Olá [[name]],<br>"
                        + "Por favor clique no link abaixo para verificar sua conta: <br>"
                + "<h3><a href=\"[[URL]]\" target=\"_self\">VERIFICAR</a></h3>"
                + "Obrigado, <br>"
                + "Forum Hub :).", usuario.getNomeCompleto(), URL_SITE + "/verificar-conta?codigo=" + usuario.getTokenIdentificador());

        enviarEmail(usuario.getUsername(), asssunto, conteudo);
    }
